...
(defn expand-pair
  ...
  (let [dir (direction a b)
    ...
    (take-while (complement #(= end %)) (iterate #(vec (map + % dir)) a))))

(defn expand-parents
  "Take a set of parents generated by jumps and expand them to single steps"
  [parents]
  (letfn [(to-hash [expanded]
            (map #(apply hash-map %) (partition 2 1 expanded)))
          (join-maps [maps] (apply merge-with into {} maps))]
    (let [expanded-pairs (map (fn [[a b]] (expand-pair a b)) parents)]
      (join-maps (map join-maps (map to-hash expanded-pairs))))))
